...
const RatingsRec: React.FC<RatingsRecProps> = ({ statements, courseData }) => {
  ...
  return (
    <Box sx={{
      height: 'calc(100% - 56px)', // Adjust for any fixed headers
      display: 'flex',
      ...
      {lowRatedActivities.length > 0 ? (
        lowRatedActivities.map((activity, index) => {
          // Fetch the difficulty and learning time values
          ...
          // Parse the typical learning time using the utility function
          ...
          return (
            ...
            >
              <Box sx={{
                ...
                >
                  Rating: {(activity.averageScore).toFixed(2)}
                </Typography>
              </Box>
