...
const StudentGradeRec: React.FC<StudentGradeRecProps> = ({ statements }) => {
  ...
  const studentGrades = useMemo(() => {
    ...
    scoredStatements.forEach((statement) => {
      ...
      if (studentEmail && score !== undefined) {
        if (!gradesMap[studentEmail]) {
          gradesMap[studentEmail] = { totalScore: 0, count: 0 };
        }
        gradesMap[studentEmail].totalScore += score;
        gradesMap[studentEmail].count += 1;
      }
    });
        ...
        studentGrades.students.map((student, index) => (
          ...
          >
            <Box sx={{
              ...
              >
                Average Grade: {student.averageScore.toFixed(2)}
              </Typography>
            </Box>
